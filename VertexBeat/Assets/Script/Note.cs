...
public class Note : MonoBehaviour
{
    ...
    public void NoteMove(GameObject[] target, ref int target_idx, int currentShape, ref bool isPassed, ref bool changeShape, int target_cnt)
    {
        Cursor.transform.position = Vector2.MoveTowards(Cursor.transform.position, target[target_idx].transform.position, bpm * noteSpeed * Time.deltaTime); // 노트의 움직임은 deltatime
        if (NoteData.instance.isNextNote) // 1/8 박자마다 메트로놈 작동
        {
            cnt++; // cnt 변수를 이용해 1/4 , 1/2 , 1박자 표현 가능
            NoteData.instance.isNextNote = false;
        }
        if (target_cnt == cnt) // target_cnt에 도달하면 박자 재생
        {
